...
def rerun_analysis_from_history() -> None:
    ...
    working_df = st.session_state.filter_history[-1]["df"].copy()
...
def go_to_column_by_name(column_name: str) -> None:
    results = st.session_state.analysis_results
    cols = results["columns_to_analyze"]

    if column_name in cols:
        st.session_state.current_column_index = cols.index(column_name)
        st.session_state.analysis_view = "detail"
        st.session_state.chart_mode = None


def go_to_next_column() -> None:
    results = st.session_state.analysis_results
    cols = results["columns_to_analyze"]
    ...
    current = st.session_state.current_column_index
    if current < len(cols) - 1:
