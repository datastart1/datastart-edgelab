...
def prepare_sorted_dataframe(df: pd.DataFrame, event_date_col: str) -> pd.DataFrame:
    df_sorted = df.copy()
    df_sorted[event_date_col] = pd.to_datetime(
        df_sorted[event_date_col],
        errors="coerce",
        dayfirst=True,
    )
...
def build_column_category_plan(
    ...
    if selected_type == "Numeric Discrete":
        return build_discrete_category_plan(df[column_name]), "unique_values"

    if selected_type == "Numeric Continuous":
...
def calculate_grand_totals(
    ...
    if runs > 0:
        ...
        cumulative_pl = profit_valid.cumsum()

        if float(starting_bank) > 0:
            equity = cumulative_pl + float(starting_bank)
            running_peak = pd.concat([
                pd.Series([float(starting_bank)]),
                equity.reset_index(drop=True),
            ], ignore_index=True).cummax().iloc[1:]
            running_peak.index = equity.index
        else:
            equity = cumulative_pl
            running_peak = equity.cummax()

        drawdown_amount = running_peak - equity
        if not drawdown_amount.empty:
            max_drawdown = float(drawdown_amount.max())

        valid_peak_mask = running_peak > 0
        if valid_peak_mask.any():
            drawdown_pct = pd.Series(0.0, index=equity.index)
            drawdown_pct.loc[valid_peak_mask] = (
                (running_peak.loc[valid_peak_mask] - equity.loc[valid_peak_mask])
                / running_peak.loc[valid_peak_mask]
            ) * 100.0
...
def get_columns_to_analyze(
    ...
    return [
        ...
        and column_types.get(col) in ["Numeric Continuous", "Numeric Discrete", "Categorical"]
    ]
...
def build_stage_d_results(
    ...
    for col in columns_to_analyze:
        if column_types.get(col) not in ["Numeric Continuous", "Numeric Discrete"]:
            continue
...
def build_stage_d_curve_data(
    df_sorted: pd.DataFrame,
    column_name: str,
    profit_col: str,
) -> pd.DataFrame:
    working = df_sorted[[column_name, profit_col]].copy()
    working[column_name] = pd.to_numeric(working[column_name], errors="coerce")
    working[profit_col] = pd.to_numeric(working[profit_col], errors="coerce")
    working = working.dropna(subset=[column_name, profit_col]).sort_values(by=column_name, ascending=True).reset_index(drop=True)

    if working.empty:
        return pd.DataFrame(columns=[column_name, profit_col, "cum_pl"])
...
def run_full_analysis(
    ...
    grand_totals = calculate_grand_totals(df_sorted, stake_col, target_col, starting_bank=starting_bank)
